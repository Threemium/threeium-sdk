...
function parseSignaturesCsv(value) {
    return value
        .split(',')
        .map((s) => s.trim())
        .filter((s) => s.length > 0);
...
describe('yield', () => {
    it('computes totals from real on-chain transaction fees (no send)', async () => {
        ...
        const txA = await connection.getTransaction(sigs[0], { commitment: 'confirmed', maxSupportedTransactionVersion: 0 });
        const txB = await connection.getTransaction(sigs[1], { commitment: 'confirmed', maxSupportedTransactionVersion: 0 });
        ...
        const report = calc.compute({
            ...
            executionFees: [{ slot: txA.slot, signature: sigs[0], lamports: feeA, description: 'transaction fee (RPC meta.fee)' }],
            protocolRevenue: [{ slot: txB.slot, signature: sigs[1], lamports: feeB, description: 'transaction fee used as an input amount' }]
        });
