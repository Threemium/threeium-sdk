...
describe('execution', () => {
  it('builds a deterministic plan and composes a v0 transaction (no send)', async () => {
    ...
    const ixA = new TransactionInstruction({ programId: programA, keys: [], data: Buffer.from([1, 2, 3]) });
    const ixB = new TransactionInstruction({ programId: programB, keys: [], data: Buffer.from([1, 2, 3]) });

    const plan = router.buildPlan({ instructions: [ixB, ixA] });
    expect(plan.length).toBe(2);
    const first = plan[0]!;
    expect(first.programId.toBase58()).toBe(programA.toBase58());
